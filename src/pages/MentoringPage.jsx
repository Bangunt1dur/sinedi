...
import { ChevronLeft, Calendar, Clock, Star, MapPin, CheckCircle, Users, X, Search } from 'lucide-react';
import { useApp } from '../context/AppContext';
import NotificationModal from '../components/NotificationModal';

export default function MentoringPage() {
    const { createOrder, user } = useApp(); // Use global user
    ...
    useEffect(() => {
        const fetchTutors = async () => {
            try {
                // Query: role == 'tutor'
                // Optional: && isVerified == true (if you add verification logic later)
                const q = query(
                    collection(db, 'users'),
                    where("role", "==", "tutor"),
                    where("isMentoringActive", "==", true)
                );
                ...
                if (tutorsData.length === 0) {
                    console.warn("DEBUG: No ACTIVE tutors found. Creating dummy data or check credentials.");
                }
    ...
    const confirmBooking = () => {
        ...
        const price = selectedTutor.tutorProfile?.price || selectedTutor.price || 50000;
        const totalBasePrice = price * sessions;
        ...
        <div className="p-6 space-y-6 animate-in fade-in pb-24">
            ...
            <div className="space-y-4">
                ...
                <div className="relative">
                    ...
                    <input
                        type="text"
                        placeholder="Cari tutor atau pelajaran..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full bg-white border border-slate-200 rounded-xl py-3 pl-12 pr-4 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-transparent transition-all shadow-sm"
                    />
                </div>
                ...
                {(() => {
                    const filteredTutors = tutors.filter(tutor => {
                        ...
                        const subjects = tutor.subjects || tutor.tutorProfile?.subjects || [];
                        const subjectMatch = Array.isArray(subjects)
                    ...
                    if (filteredTutors.length === 0) {
                        return (
                            <div className="text-center py-10 text-slate-400">
                                {searchTerm ? 'Tutor tidak ditemukan.' : 'Belum ada tutor tersedia saat ini.'}
                            </div>
                        );
                    }
                    ...
                    return filteredTutors.map(tutor => (
                        <div key={tutor.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4 group hover:border-yellow-400 transition-all">
                            <div className={`w-14 h-14 rounded-full ${tutor.color || 'bg-slate-200'} flex items-center justify-center text-slate-700 font-bold text-xl overflow-hidden`}>
                                {tutor.photo ? (
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex flex-wrap gap-1 mt-1">
                                    {/* Handle subjects array or string fallback */}
                                ...
                                <div className="flex items-center gap-3 mt-2">
                                    <div className="flex items-center gap-1 text-yellow-500">
                                        ...
                                        <span className="text-xs font-bold text-slate-700">
                                            {tutor.rating ? Number(tutor.rating).toFixed(1) : 'New'}
                                            {tutor.ratingCount > 0 && <span className="text-slate-400 font-normal ml-1">({tutor.ratingCount})</span>}
                                        </span>
                                    </div>
                                    <span className="text-xs font-bold text-slate-900">Rp {(tutor.tutorProfile?.price || tutor.price || 0).toLocaleString()}/sesi</span>
                                </div>
                            </div>
                            <button
                                ...
                                <div className="flex justify-between items-center mb-4">
                                    <div>
                                        ...
                                        <p className="text-xl font-bold text-slate-900">
                                            Rp {(() => {
                                                const total = (selectedTutor.tutorProfile?.price || selectedTutor.price || 0) * (selectedSlots.length || 0);
