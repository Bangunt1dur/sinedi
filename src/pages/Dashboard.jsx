...
export default function Dashboard() {
    ...
    // Guard: Context not found or Loading
    if (!context) {
        return <div className="flex h-screen items-center justify-center">Loading Application...</div>;
    }

    const { user, orders, notifications, markNotificationRead, takeJob, createOrder, updateOrder, submitReview, tutorStats } = context;

    // ADMIN OVERRIDE: Redirect to standalone Admin Page (No Sidebar)
    if (user && user.isAdmin) {
        return <Navigate to="/admin/withdraw" replace />;
    }
    ...
    // 2. Error Handling: Loading State (If user not loaded yet)
    if (!user) {
        return (
    ...
    return (
        <div className="p-6 space-y-8 animate-in fade-in pb-32" onClick={() => setShowNotif(false)}>
                            ...
                            <div className="max-h-64 overflow-y-auto">
                                {notifications && notifications.length === 0 ? (
                                    ...
                                    <div
                                        ...
                                        className={`p-4 hover:bg-slate-50 transition-colors flex gap-3 border-b border-slate-50 last:border-0 cursor-pointer ${!n.isRead ? 'bg-yellow-50/50' : ''}`}
                                    >
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-slate-400 mt-2 text-right">
                                                {n.createdAt ? new Date(n.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Baru saja'}
                                            </p>
                                        </div>
                                    </div>
                                ))}
            ...
            {
                user.isAdmin && (
                    <div onClick={() => navigate('/admin/withdraw')} className="mb-6 bg-slate-900 mx-6 mt-4 p-4 rounded-xl flex items-center justify-between shadow-lg cursor-pointer hover:bg-slate-800 transition-colors">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-red-500 rounded-full flex items-center justify-center text-white">
                                <AlertCircle className="w-6 h-6" />
                            </div>
                            <div>
                                <h3 className="font-bold text-white">Admin Center</h3>
                                <p className="text-xs text-slate-400">Kelola Penarikan & Bukti Transfer</p>
                            </div>
                        </div>
                    </div>
                )
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            >
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-sm font-medium text-slate-400">Saldo Aktif</span>
                                </div>
                                <h2 className="text-2xl font-bold text-slate-900">Rp {(user?.wallet || 0).toLocaleString('id-ID')}</h2>
                                <p className="text-xs text-green-600 font-bold mt-1">+ Real-time Sync</p>
                            </div>
                            ...
                            >
                                ...
                                <h2 className="text-2xl font-bold text-slate-900">{tutorStats?.averageRating || 'New'} <span className="text-sm text-slate-400 font-normal">/ 5.0</span></h2>
                                <p className="text-xs text-slate-400 mt-1">Berdasarkan {tutorStats?.totalReviews || 0} review</p>
                            </div>
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                                ...
                                <h2 className="text-2xl font-bold text-slate-900">{orders ? orders.filter(o => o.status === 'In Progress' && o.tutorName === user.name).length : 0} Tasks</h2>
            ...
            <ReviewModal
                ...
                onSubmit={(data) => {
                    ...
                    setNotification({ show: true, type: 'success', message: 'Terima kasih atas ulasanmu!' });
                }}
            />
        </div >
    );
